...
async def process_panel_action(interaction: discord.Interaction, panel_name: str, command: str, args: list, modal_data: dict = None):
    ...
    try:
        ...
        if msg:
            ...
            # Fallback: If dynamic panel config was lost (e.g. bot restarted), preserve original msg text
            if not panel_cfg and msg.content:
                content = msg.content
                # Strip out old status line if it exists
                if "\nLast: `" in content:
            ...
            if PANEL_STATUS_LINE:
                ...
                content = f"{safe_content}\nLast: `{last_cmd}` • {user_display} • {ts}".strip()

                # Protect against Discord's 2000 character limit for message content
                if len(content) > 2000:
